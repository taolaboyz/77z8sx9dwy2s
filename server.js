...
// ===== BIẾN TOÀN CỤC =====
let lastData = null;
let history = []; // lưu {phien, du_doan, ket_qua, danh_gia}

// ===== GỌI API GỐC =====
async function fetchSicboSunWin() {
  try {
    const res = await fetch("https://sicokk.onrender.com/predict"); // dùng global.fetch
    ...
    if (!lastData || current.Phien !== lastData.Phien) {
      if (lastData) {
        // Đánh giá đúng/sai dự đoán của phiên trước so với KQ thực tế
        const danh_gia =
