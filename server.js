...
async function fetchSicboSunWin() {
  try {
    const res = await axios.get("https://sicsunnemaycu-du3z.onrender.com/predict");
    const data = res.data;

    // Chuẩn hóa dữ liệu (KHÔNG ĐẢO NGƯỢC)
    const current = {
      Phien: data.Phien,
      ...
      du_doan: data.du_doan,             // dữ liệu gốc
      dudoan_vi: data.dudoan_vi,         // dữ liệu gốc
