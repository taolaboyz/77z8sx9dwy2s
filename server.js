...
async function fetchSicboSunWin() {
  try {
    ...
    const current = {
      ...
      du_doan_nguoc: reversePrediction(data.du_doan),       // giữ lại đảo
      dudoan_vi_nguoc: reversePrediction(data.dudoan_vi),   // giữ lại đảo
    ...
    if (!lastData || current.Phien !== lastData.Phien) {
      if (lastData) {
        ...
        history.unshift({
          ...
          du_doan_nguoc: lastData.du_doan_nguoc,       // lưu ngược
          dudoan_vi_nguoc: lastData.dudoan_vi_nguoc,   // lưu ngược
...
// API cho frontend (khớp với code JS bạn đưa)
...
// API trả về lịch sử (có cả dự đoán ngược)
